...
describe("DemandAggregator", () => {
  ...
  afterEach(() => {
    db.close();
    try {
      unlinkSync("./swap.purchasing.test.db");
    } catch {}
  });

  it("correctly sums demand across members", async () => {
    aggregator.addPledge({
      memberId: "alice",
      provider: "anthropic",
      estimatedMonthlyTokens: 500_000,
      maxPricePerMToken: 2.0,
      commitmentMonths: 3,
    });
    aggregator.addPledge({
      memberId: "bob",
      provider: "anthropic",
      estimatedMonthlyTokens: 1_200_000,
      maxPricePerMToken: 1.5,
      commitmentMonths: 12,
    });
  ...
  it("calculates volume weighted blended maximum price", async () => {
    aggregator.addPledge({
      memberId: "alice",
      provider: "google",
      estimatedMonthlyTokens: 100_000,
      maxPricePerMToken: 2.0,
      commitmentMonths: 1,
    });
    aggregator.addPledge({
      memberId: "bob",
      provider: "google",
      estimatedMonthlyTokens: 300_000,
      maxPricePerMToken: 1.0,
      commitmentMonths: 1,
    });
