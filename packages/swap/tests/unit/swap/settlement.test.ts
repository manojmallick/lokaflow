...
describe("TradeSettlement", () => {
  ...
  it("rolls back both sides if one deduction fails due to insufficient balance", async () => {
    ...
    const offer = makeListing(
      "alice",
      { type: "lokacredits", amount: 50 },
      { type: "api-credits", provider: "anthropic", amount: 100 },
    );
    const request = makeListing(
      "bob",
      { type: "api-credits", provider: "anthropic", amount: 100 },
      { type: "lokacredits", amount: 50 },
    );
  ...
  it("is idempotent — settling twice does not double-credit", async () => {
    ...
    await settlement.seedBalance("dave", {
      type: "api-credits",
      provider: "google",
      amount: 25000,
    });
    ...
    const offer = makeListing(
      "charlie",
      { type: "lokacredits", amount: 500 },
      { type: "api-credits", provider: "google", amount: 1000 },
    );
    const request = makeListing(
      "dave",
      { type: "api-credits", provider: "google", amount: 1000 },
