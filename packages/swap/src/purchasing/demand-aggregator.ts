    ...
    this.db
     ...
     `,
      ...
      .run(
        pledge.memberId,
        pledge.provider,
        pledge.estimatedMonthlyTokens,
        pledge.maxPricePerMToken,
        pledge.commitmentMonths,
      );
  }

  getPledges(provider: string): MemberDemandPledge[] {
    return this.db
      .prepare(`SELECT * FROM demand_pledges WHERE provider = ?`)
      .all(provider)
      .map((r: any) => ({
  ...
  async getMonthlyDemand(provider: string): Promise<AggregateDemand> {
    ...
    return {
      ...
      tierBreakdown: {
        ...
        committed3Month: pledges.filter((p) => p.commitmentMonths >= 3 && p.commitmentMonths < 12)
          .length,
