...
export class PoolUsageTracker {
  ...
  async recordUsage(poolId: string, memberId: string, tokensUsed: number): Promise<void> {
    ...
    const alloc = this.db
      .prepare(
        `SELECT allocated, used FROM member_pool_allocations WHERE pool_id = ? AND member_id = ?`,
      )
      .get(poolId, memberId) as any;

    if (!alloc)
      throw new Error(`[LokaSwap] Member ${memberId} is not subscrubed to pool ${poolId}`);
    ...
    this.db.transaction(() => {
      ...
      this.db
        .prepare(
          `UPDATE member_pool_allocations SET used = used + ? WHERE pool_id = ? AND member_id = ?`,
        )
        .run(tokensUsed, poolId, memberId);
      // Deduct from pool global cap
      this.db
        .prepare(`UPDATE api_pools SET total_used = total_used + ? WHERE id = ?`)
