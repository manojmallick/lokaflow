...
export class ChargeGuardian {
  ...
  async enforce(
    state: BatteryState & { agent: any },
    policy: BatteryPolicy,
  ): Promise<ChargeAction> {
    ...
    if (windowOverride) {
      ...
      if (state.agent.supportsChargeControl) {
        ...
        return {
          action: "window-override",
          message: `Schedule window active: ${windowOverride.name}, charging to ${windowLimit}%`,
        };
      } else {
        return {
          action: "alert",
          message: `Schedule window active: ${windowOverride.name}. Unplug above ${windowLimit}%.`,
        };
      }
    }
    ...
    if (state.isCharging && state.percentCharge >= limit) {
      if (state.agent.supportsChargeControl) {
        ...
        return {
          action: "alert",
          message: `Battery at ${state.percentCharge.toFixed(0)}%, above ${limit}% limit — unplug to protect battery`,
        };
      }
    }
