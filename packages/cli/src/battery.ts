...
function readLinuxBattery(nodeId: string) {
  try {
    ...
    const rd = (f: string) => {
      try {
        return readFileSync(`${base}/${f}`, "utf-8").trim();
      } catch {
        return null;
      }
    };
...
export const batteryCommand = new Command("battery")
  ...
  .action(async () => {
    ...
    try {
      ...
      const { LokaMesh, ClusterBatteryStore, HealthTracker, BatteryReport, calculateStressScore } =
      ...
      for (const node of status.nodes) {
        let raw =
          platform === "darwin"
            ? readMacosBattery(node.id)
            : platform === "linux"
              ? readLinuxBattery(node.id)
              : null;
        if (raw) {
      ...
      if (states.length === 0) {
        ...
        console.log(
          chalk.gray("  (Desktop / offline nodes are skipped; Windows not supported yet)"),
        );
      } else {
