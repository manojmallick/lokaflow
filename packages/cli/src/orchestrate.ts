  ...
  .action(async (query: string, options) => {
    ...
    try {
      const [
        ...
        { loadConfig, OllamaProvider, ClaudeProvider, OpenAIProvider },
      ] = await Promise.all([import("@lokaflow/orchestrator"), import("@lokaflow/core")]);
      ...
      if (options.dryRun) {
        ...
        for (const node of graph.nodes) {
          const deps = node.dependsOn.length
            ? chalk.gray(` (after: ${node.dependsOn.join(", ")})`)
            : "";
