...
function stateColor(state: string): string {
  switch (state) {
    case "online":
      return chalk.green(state.padEnd(12));
    case "busy":
      return chalk.cyan(state.padEnd(12));
    case "light_sleep":
    case "deep_sleep":
      return chalk.yellow(state.padEnd(12));
    case "waking":
      return chalk.blue(state.padEnd(12));
    default:
      return chalk.red((state || "unknown").padEnd(12));
  }
}
    ...
    try {
      ...
      if (options.wake) {
        ...
        if (woken) {
          ...
          console.error(
            chalk.red(`  Node '${options.wake}' not found or has no MAC address configured.`),
          );
        }
        await mesh.stop();
        return;
