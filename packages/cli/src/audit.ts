    ...
    .action(async (file: string, options) => {
        ...
        try {
            ...
            if (!provider) {
                ...
                console.error(chalk.gray("  Supported: ChatGPT (conversations.json with 'mapping') or Claude (with 'chat_messages')."));
                return;
            }
            ...
            const report = await engine.analyze(data, Number.isFinite(subscriptionEur) ? subscriptionEur : 22.99);

            // Print report
            console.log(chalk.bold(`\n════════════════════════════════════════════════════════`));
            console.log(chalk.bold(`  LokaAudit™ — Subscription Analysis`));
            console.log(chalk.gray(`  Provider: ${report.provider}  ·  Period: ${report.periodDays} days`));
            console.log(chalk.bold(`════════════════════════════════════════════════════════\n`));
            ...
            console.log(`  Tokens estimated       : ${chalk.white(report.totalTokensEstimated.toLocaleString())}`);
            console.log(`  Simple (→ local free)  : ${chalk.green(report.simpleQueriesCount)} (${Math.round((report.simpleQueriesCount / Math.max(1, report.totalUserMessages)) * 100)}%)`);
            console.log(`  Complex (→ cloud API)  : ${chalk.yellow(report.complexQueriesCount)}`);
            console.log();
            console.log(`  Your subscription      : ${chalk.white("€" + report.currentMonthlySubscriptionEur + "/mo")}`);
            console.log(`  LokaFlow equivalent    : ${chalk.white("€" + report.lokaflowEquivalentCostEur + "/mo")}`);

            if (report.canCancel) {
                console.log(`  Monthly savings        : ${chalk.green("€" + report.monthlySavingsEur + "/mo")} ✓`);
                console.log();
                console.log(chalk.green("  ✅ You can cancel your subscription and save money with LokaFlow."));
            } else {
                console.log(`  Monthly difference     : ${chalk.red("+" + Math.abs(report.monthlySavingsEur) + "/mo")}`);
                console.log();
                console.log(chalk.yellow("  ℹ️  Keep your current subscription — you're a heavy complex-query user."));
            }
